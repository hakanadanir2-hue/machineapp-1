...
const SERVICES = [
  { id: "deneme",    label: "Ücretsiz Deneme Antrenmanı", paid: false },
  { id: "fitness",   label: "Fitness Üyelik Danışma",     paid: false },
  { id: "pt",        label: "Personal Trainer",           paid: true  },
  { id: "boks",      label: "Boks Özel Ders",             paid: true  },
  { id: "kickboks",  label: "Kickboks",                   paid: true  },
  { id: "muay_thai", label: "Muay Thai",                  paid: true  },
];
...
export default function RandevuPage() {
  ...
  const [waNum, setWaNum]           = useState(WA);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    const [{ data: waSetting }, { error: leadErr }] = await Promise.all([
      supabase.from("site_settings").select("value").eq("key","contact_whatsapp").single(),
      supabase.from("leads").insert({
        type: "appointment", status: "new", is_read: false,
        name: form.full_name, email: form.email, phone: form.phone,
        appt_date: dateStr, appt_time: selectedTime, appt_service: form.service,
        appt_notes: form.notes || null,
        message: `Randevu: ${SERVICES.find(s=>s.id===form.service)?.label??form.service} — ${dateStr} ${selectedTime}`,
      }),
    ]);
    if (waSetting?.value) setWaNum(String(waSetting.value).replace(/\D/g,""));
    if (leadErr) {
  ...
  const isPaid = selectedService?.paid ?? false;

  const waConfirmMsg = success && selected
    ? encodeURIComponent(
  ...
  const waReceiptMsg = success
    ? encodeURIComponent(
        `Merhaba, ${form.full_name} — ${selectedService?.label ?? form.service} randevusu için ödeme dekontumu iletiyorum.`
      )
    : "";
        ...
        <main style={{ minHeight:"100vh", background:"#0B0B0B", display:"flex", alignItems:"center", justifyContent:"center", padding:"5rem 1rem" }}>
          <div style={{ textAlign:"center", maxWidth:520 }}>
            ...
            <p style={{ color:"rgba(255,255,255,0.45)", fontSize:"0.875rem", marginBottom:"2rem" }}>
              Hizmet: <strong style={{ color:"#fff" }}>{selectedService?.label}</strong>
            </p>

            <div style={{ display:"flex", flexDirection:"column", gap:"0.75rem", marginBottom:"1.5rem" }}>
              <a href={`https://wa.me/${waNum}?text=${waConfirmMsg}`} target="_blank" rel="noopener noreferrer"
              ...
              {isPaid && (
                <a href={`https://wa.me/${waNum}?text=${waReceiptMsg}`} target="_blank" rel="noopener noreferrer"
                  style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:"0.625rem", padding:"0.875rem 1.5rem", background:"rgba(212,175,55,0.12)", color:"#D4AF37", fontWeight:700, fontSize:"0.9375rem", borderRadius:14, textDecoration:"none", border:"1px solid rgba(212,175,55,0.3)" }}>
            ...
            {isPaid && (
              <div style={{ padding:"0.875rem 1rem", background:"rgba(212,175,55,0.07)", border:"1px solid rgba(212,175,55,0.2)", borderRadius:12, fontSize:"0.8125rem", color:"rgba(255,255,255,0.55)", lineHeight:1.6 }}>
                Ödemenizi yaptıktan sonra dekontunuzu yukarıdaki buton ile WhatsApp üzerinden iletiniz.
                Randevunuz dekont onayından sonra kesinleşecektir.
              </div>
            )}
          </div>
        </main>
        ...
        <div className="page-container">
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr", gap:"1.5rem", maxWidth:900, marginInline:"auto" }} className="randevu-grid">
            ...
            <div style={{ background:"#141414", borderRadius:20, border:"1px solid rgba(255,255,255,0.07)", padding:"1.5rem" }}>
              ...
              <DayPicker mode="single" selected={selected} onSelect={setSelected} locale={tr} disabled={{ before: new Date() }} />
            ...
            <div style={{ background:"#141414", borderRadius:20, border:"1px solid rgba(255,255,255,0.07)", padding:"1.5rem" }}>
              ...
              {error && <div style={{ marginBottom:"1rem", padding:"10px 14px", background:"rgba(248,113,113,0.1)", border:"1px solid rgba(248,113,113,0.25)", borderRadius:10, color:"#f87171", fontSize:13 }}>{error}</div>}
              ...
              <form onSubmit={onSubmit} style={{ display:"flex", flexDirection:"column", gap:"1rem" }}>
                {([
                  ...
                  <div key={f.k}>
                    ...
                    <input value={form[f.k]} onChange={e=>set(f.k,e.target.value)} type={f.t} placeholder={f.p} style={INP} />
                ...
                <div>
                  ...
                  <select value={form.service} onChange={e=>set("service",e.target.value)}
                    style={{ ...INP, color:"rgba(255,255,255,0.8)", cursor:"pointer" }}>
                    <option value="">Seçin...</option>
                    {SERVICES.map(s => <option key={s.id} value={s.id}>{s.label}{s.paid ? " (Ücretli)" : " (Ücretsiz)"}</option>)}
                  </select>
                  {errs.service && <p style={{ color:"#f87171", fontSize:"0.75rem", marginTop:4 }}>{errs.service}</p>}
                </div>

                {isPaid && (
                  <div style={{ padding:"0.75rem 1rem", background:"rgba(212,175,55,0.07)", border:"1px solid rgba(212,175,55,0.2)", borderRadius:10, fontSize:"0.8125rem", color:"rgba(255,255,255,0.6)", lineHeight:1.6 }}>
                    Bu ücretli bir hizmettir. Randevunuzu onayladıktan sonra ödeme dekontunuzu WhatsApp üzerinden iletmeniz gerekmektedir.
                  </div>
                )}
