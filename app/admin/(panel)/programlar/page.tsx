  ...
  return (
    <div style={{ padding:24, background:"#0A0A0A", minHeight:"100vh", color:"#fff" }}>
      ...
      <p style={{ color:"rgba(255,255,255,.3)", fontSize:13, marginBottom:24 }}>
        AI tarafından oluşturulan kişisel programları incele ve onayla
      </p>
      ...
      {loading ? (
        ...
        <div style={{ color:"rgba(255,255,255,.2)", padding:40, textAlign:"center", background:"#111", borderRadius:12, border:"1px solid rgba(255,255,255,.05)" }}>
          Henüz program yok
        </div>
      ) : (
                  ...
                  <div style={{ flex:1, minWidth:200 }}>
                    ...
                    {prof && (
                      <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
                        {prof.full_name && <span style={{ fontSize:11, color:"rgba(255,255,255,.4)" }}><User size={10} style={{ display:"inline" }} /> {prof.full_name}</span>}
                        {prof.age && <span style={{ fontSize:11, color:"rgba(255,255,255,.4)" }}>{prof.age} yaş · {prof.gender === "erkek" ? "E" : "K"}</span>}
                        {prof.bmi && <span style={{ fontSize:11, color:"rgba(255,255,255,.4)" }}>BMI: {prof.bmi} ({prof.bmi_category})</span>}
                    ...
                    <div style={{ fontSize:10, color:"rgba(255,255,255,.2)", marginTop:6 }}>
                      {new Date(p.created_at).toLocaleString("tr-TR")} · {p.days_per_week} gün/hafta · {p.duration_weeks} hafta
                    </div>
                  </div>
      ...
      {detailId && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.95)", zIndex:200, overflowY:"auto", padding:"20px 16px" }}
          ...
          <div style={{ maxWidth:760, margin:"0 auto", background:"#111", border:"1px solid rgba(255,255,255,.1)", borderRadius:20, overflow:"hidden" }}>
